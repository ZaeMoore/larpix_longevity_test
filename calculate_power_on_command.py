...
def main(healthy, mode):
    ...
    if mode == 'nominal':
        ...
        for tile_number in healthy:
            string_tile_number += f'{tile_number},'
    ...
    if mode == 'accelerated':
        ...
        for tile_number in healthy:
            # Array format
            list_tile_number.append(list_tiles[tile_number-1][0])
            list_vdda.append(calc_vdda(list_tiles[tile_number-1][1]))
            list_vddd.append(calc_vddd(list_tiles[tile_number-1][2]))
            # String format
            string_tile_number += f'{list_tiles[tile_number-1][0]},'
            string_vdda += f'{int(calc_vdda(list_tiles[tile_number-1][1]))},'
            string_vddd += f'{int(calc_vddd(list_tiles[tile_number-1][2]))},'
